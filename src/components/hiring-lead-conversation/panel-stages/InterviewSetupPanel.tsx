...
import {
  ...
  Plus, Minus, Trash2, Clock, ListChecks, Sparkles, MessageSquarePlus,
} from 'lucide-react';
...
export function InterviewSetupPanel() {
  ...
  const coreBarPct = interviewSetup.totalDurationMins > 0
    ? Math.min(100, (coreMins / interviewSetup.totalDurationMins) * 100) : 0;
    ...
    const ai: CoreQuestion[] = [
      {
        ...
        estimatedMinutes: 4, type: 'scenario', source: 'ai',
      },
      {
        id: `q-ai-${ts}-2`,
        ...
        estimatedMinutes: 2, type: 'knowledge', source: 'ai',
      },
      {
        id: `q-ai-${ts}-3`,
        ...
        estimatedMinutes: 5, type: 'scenario', source: 'ai',
      },
    ];
  ...
  const handleAddManual = () => {
    ...
    const q: CoreQuestion = {
      ...
      type: manualType, source: 'manual',
    };
    updateInterviewSetup({ coreQuestions: [...interviewSetup.coreQuestions, q] });
    setManualText(''); setManualAnswerKey(''); setManualType('scenario'); setIsAddingManually(false);
  };
        ...
        <div className="grid grid-cols-2 gap-3">
          <button type="button" onClick={() => updateInterviewSetup({ includeAIQuestions: true })}
            className={`relative text-left rounded-xl border-2 p-4 transition-all duration-150 ${
              interviewSetup.includeAIQuestions ? 'border-[#7800D3] bg-[#faf5ff] shadow-sm' : 'border-border bg-white hover:bg-muted/30 hover:border-muted-foreground/30'
            }`}>
            {interviewSetup.includeAIQuestions && <CheckCircle2 className="absolute top-3 right-3 h-4 w-4 text-[#7800D3]" />}
            <div className={`inline-flex items-center justify-center h-9 w-9 rounded-lg mb-3 ${interviewSetup.includeAIQuestions ? 'bg-[#7800D3]/10' : 'bg-muted'}`}>
              <Sparkles className={`h-5 w-5 ${interviewSetup.includeAIQuestions ? 'text-[#7800D3]' : 'text-muted-foreground'}`} />
            </div>
            <p className={`text-sm font-semibold mb-1 ${interviewSetup.includeAIQuestions ? 'text-[#7800D3]' : 'text-foreground'}`}>AI Assisted</p>
            <p className="text-xs text-muted-foreground leading-snug">AI helps structure the interview with guided question planning and adaptive follow-ups.</p>
          </button>

          <button type="button" onClick={() => updateInterviewSetup({ includeAIQuestions: false })}
            className={`relative text-left rounded-xl border-2 p-4 transition-all duration-150 ${
              !interviewSetup.includeAIQuestions ? 'border-slate-500 bg-slate-50 shadow-sm' : 'border-border bg-white hover:bg-muted/30 hover:border-muted-foreground/30'
            }`}>
            {!interviewSetup.includeAIQuestions && <CheckCircle2 className="absolute top-3 right-3 h-4 w-4 text-slate-600" />}
            <div className={`inline-flex items-center justify-center h-9 w-9 rounded-lg mb-3 ${!interviewSetup.includeAIQuestions ? 'bg-slate-100' : 'bg-muted'}`}>
              <ListChecks className={`h-5 w-5 ${!interviewSetup.includeAIQuestions ? 'text-slate-600' : 'text-muted-foreground'}`} />
            </div>
            <p className={`text-sm font-semibold mb-1 ${!interviewSetup.includeAIQuestions ? 'text-slate-700' : 'text-foreground'}`}>Manual Setup</p>
            <p className="text-xs text-muted-foreground leading-snug">Build and manage the interview flow yourself with full control.</p>
          </button>
        </div>
      </Card>
      ...
      <Card className="p-5 border border-border">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            {interviewSetup.includeAIQuestions && (
              <Button type="button" size="sm" variant="outline" onClick={handleGenerateWithAI}
            ...
            <Button type="button" size="sm" variant="outline" onClick={() => setIsAddingManually(true)} className="text-xs" disabled={isAddingManually}>
        ...
        {interviewSetup.coreQuestions.length === 0 && !isAddingManually && (
          <div className="flex flex-col items-center gap-2 py-8 text-muted-foreground">
            ...
            <p className="text-sm">{interviewSetup.includeAIQuestions ? 'No questions yet — add your own or let AI suggest some' : 'No questions yet — add your own to get started'}</p>
          </div>
        )}
        ...
        <div className="space-y-3">
            ...
            <Card key={q.id} className="p-3 bg-muted/20 border border-border/60">
              <div className="flex items-start gap-2">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <Button type="button" variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-red-500" onClick={() => handleDelete(q.id)}>
          ...
          {isAddingManually && (
            <Card className="p-4 border-2 border-dashed border-[#7800D3]/30 bg-[#faf5ff]/50">
              <div className="space-y-3">
                <Textarea placeholder="Enter your question..." value={manualText} onChange={(e) => setManualText(e.target.value)} rows={2} className="text-sm resize-none" />
                <Textarea placeholder="Answer key (optional)..." value={manualAnswerKey} onChange={(e) => setManualAnswerKey(e.target.value)} rows={2} className="text-sm resize-none" />
                <div>
                  <p className="text-xs text-muted-foreground mb-2">Question type</p>
                  <RadioGroup value={manualType} onValueChange={(v) => setManualType(v as 'scenario' | 'knowledge')} className="flex gap-4">
                    <div className="flex items-center gap-2">
                      <RadioGroupItem value="scenario" id="type-scenario" />
                      <Label htmlFor="type-scenario" className="text-sm cursor-pointer">Scenario <span className="text-muted-foreground">(~4 min)</span></Label>
                    </div>
                    <div className="flex items-center gap-2">
                      <RadioGroupItem value="knowledge" id="type-knowledge" />
                      <Label htmlFor="type-knowledge" className="text-sm cursor-pointer">Knowledge <span className="text-muted-foreground">(~2 min)</span></Label>
                    </div>
                  </RadioGroup>
                </div>
                <div className="flex gap-2">
                  <Button type="button" size="sm" onClick={handleAddManual} className="bg-[#7800D3] hover:bg-[#6600bb] text-white">Add Question</Button>
                  <Button type="button" size="sm" variant="outline" onClick={() => { setIsAddingManually(false); setManualText(''); setManualAnswerKey(''); }}>Cancel</Button>
                </div>
              </div>
            </Card>
          )}
        ...
        {!isOverBudget && interviewSetup.totalDurationMins > 0 && (
          <div className="mb-4">
            <div className="flex h-3 w-full rounded-full overflow-hidden bg-muted/40 gap-0.5">
              {coreMins > 0 && <div className="bg-blue-500 rounded-l-full transition-all duration-300" style={{ width: `${coreBarPct}%` }} />}
              {remainingMins > 0 && showAI && <div className="bg-purple-400 rounded-r-full transition-all duration-300" style={{ width: `${remainingBarPct}%` }} />}
              {remainingMins > 0 && !showAI && <div className="bg-muted/60 rounded-r-full transition-all duration-300" style={{ width: `${remainingBarPct}%` }} />}
              {coreMins === 0 && <div className="flex-1 bg-muted/40 rounded-full" />}
            </div>
            <div className="flex items-center gap-4 mt-2 text-[10px] flex-wrap">
              <span className="flex items-center gap-1"><span className="inline-block w-2 h-2 rounded-full bg-blue-500" />Preset Questions</span>
              {showAI && <span className="flex items-center gap-1"><span className="inline-block w-2 h-2 rounded-full bg-purple-400" />Adaptive AI</span>}
              {!showAI && remainingMins > 0 && <span className="text-muted-foreground">Unused time</span>}
            </div>
          </div>
        )}

        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-sm bg-muted/30 rounded-lg px-4 py-3">
          <span><span className="text-muted-foreground text-xs">Preset </span><span className="font-semibold text-blue-600">{coreMins} min</span></span>
          {showAI && <><span className="text-muted-foreground/50">|</span><span><span className="text-muted-foreground text-xs">Adaptive AI </span><span className="font-semibold text-purple-500 italic text-xs">fills rest</span></span></>}
          <span className="text-muted-foreground/50">|</span>
          <span><span className="text-muted-foreground text-xs">Total </span><span className="font-semibold text-foreground">{interviewSetup.totalDurationMins} min</span></span>
        </div>
        <p className="mt-2 text-xs text-muted-foreground px-1">
          {showAI ? 'Preset Questions play first — Adaptive AI fills whatever time remains.' : 'Only your Preset Questions will be asked in the interview.'}
        </p>

        {isOverBudget && (
          <div className="mt-3 flex items-start gap-2 rounded-lg border border-yellow-300 bg-yellow-50 px-4 py-3">
            <AlertTriangle className="h-4 w-4 text-yellow-600 mt-0.5 shrink-0" />
            <p className="text-sm text-yellow-800">Preset questions exceed the total interview duration. Increase the time limit or remove some questions.</p>
          </div>
        )}
      </Card>

      <div className="pt-1">
        <Button onClick={handleNext} className="w-full h-12 text-base font-semibold bg-gradient-to-r from-[#503afd] to-[#3857fd] hover:from-[#503afd]/90 hover:to-[#3857fd]/90 text-white border-0">
